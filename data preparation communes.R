...
coords1 <- name_geographic_information %>% 
  dplyr::select(numéro_département, nom_département, latitude, longitude, nom_commune) %>% 
  rename(code = numéro_département, DEP_NAME = nom_département, COM_NAME = nom_commune)
#coords1 <- saveRDS(coords1, file= "coords1.rds")
coords1 <- readRDS("coords1.rds")


#preparing salary per department dataset
COM_base_cc_dads_2014 <- read_excel("COM-base-cc-dads-2014.xlsx")
df_com_sal <- COM_base_cc_dads_2014 %>% 
  dplyr::select(CODGEO, LIBGEO, SNHM14) %>% 
  rename(COM_CODE = CODGEO, COM_NAME = LIBGEO, SNHM14_com = SNHM14)

#df_com_sal <- saveRDS(df_com_sal, file= "df_com_sal.rds")
...
df_depcom_pop <- correspondance_code_insee_code_postal %>% 
  dplyr::select(Département, Population, Commune) %>% 
  rename(nom = Département, pop = Population , COM_NAME = Commune) %>% 
  mutate(nom = str_to_title(nom))  #for making capital letters to small letters


#df_depcom_pop <- saveRDS(df_depcom_pop, file= "df_depcom_pop.rds")
...
#saveRDS(df_depcom, "merged_votes_map.rds")
...
communes_age <- df_ages %>% 
  mutate(`20_24` = sum(`20_24`, na.rm = T),
...
communes_age <- communes_age %>% 
  pivot_longer(names_to = "age", values_to ="people" , -c(LIBGEO,nom))


new_age =  data.frame(age = unique(communes_age$age), 
                      average_age = c(22,27,32,37,42,47,52,57,62,67,72,77,82))

communes_age <- left_join(communes_age, new_age) 

test <- communes_age %>%  
  mutate(row = row_number()) %>% 
  pivot_wider(names_from = age, values_from = people) %>% select(-row) %>% 
  group_by(LIBGEO) %>% summarise(across(`20_24`:`80`, ~ mean(., na.rm = TRUE)))

#summarise(across(`20-24`:`80`, ~mean(.x , na.rm =T)))
#saveRDS(communes_age, "communes_age.rds") 


merged_withgeo <- readRDS("~/Desktop/Datasets/French Election/Data/merged_withgeo.rds")
niloo <- merged_withgeo %>% 
  dplyr::select(INSEE_CODE, COM_NAME, COM_CODE, DEP_NAME, DEP_CODE, 
                Cand1_LastName, Cand1_Votes, 
...
dep_vote <- niloo %>% select(-c(COM_NAME, COM_CODE, DEP_CODE)) %>% 
  pivot_longer(cols = -c(DEP_NAME, INSEE_CODE),names_to = c("set",".value"), 
               names_pattern = "^[A-Za-z]+(\\d+)_([A-Za-z]+)") %>% group_by(DEP_NAME) %>% 
...
#saveRDS(dep_vote, "dep_vote.rds")


com_vote <- niloo %>% select(-c(COM_CODE, DEP_CODE)) %>%
  pivot_longer(cols = -c(DEP_NAME, COM_NAME, INSEE_CODE), names_to = c("set",".value"), 
               names_pattern = "^[A-Za-z]+(\\d+)_([A-Za-z]+)") %>% 
  group_by(COM_NAME) %>% 
  mutate(total_votes = sum(Votes))%>% 
  group_by(COM_NAME, LastName) %>% 
  mutate(votep = sum(Votes),
         percent_vote = ifelse(total_votes == 0, 0, votep / total_votes)) %>% 
  summarise(percent_vote = min(percent_vote), 
            INSEE_CODE = unique(INSEE_CODE),
...
com_vote <- merged_withgeo %>% 
  select(INSEE_CODE, geometry) %>%  
   right_join(.,com_vote)

com_vote <- st_as_sf(com_vote)
...
com_vote <- left_join(com_vote, coords12)

#saveRDS(com_vote, "com_vote.rds")
