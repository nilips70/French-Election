...
server <- function(input, output) {
  ...
  output$view <- renderLeaflet({ #pay attention to what is the type of plot (main plot)
    ...
    pal2 <- colorFactor(palette = c("Seagreen1", "Palegreen3", "Springgreen4" ,"Lawngreen", "Cadetblue3",
                                    "Deepskyblue2", "Aquamarine4", "Yellow1", "Darkslategray1", "Navy", "Snow3"), 
                        levels = c("ARTHAUD", "ASSELINEAU", "CHEMINADE", "DUPONT-AIGNAN", "FILLON", 
                                   "HAMON", "LASSALLE", "LE PEN", "MACRON", "MÉLENCHON", "POUTOU"))
    
    
    pal3 <- colorNumeric("viridis", domain = df_merged$unemployment_rate)
    ...
    leaflet() %>%
      ...
      addLegend(pal = pal3, group = "Unemployment Rate", values = df_merged$unemployment_rate, title = "Unemployment %",
                opacity = 0.7,
                labFormat = labelFormat(suffix = " %")) %>%
    addLayersControl(
  ...
  output$view2 <- renderPlotly({
    ...
    plot1 <- df3 %>% filter(DEP_NAME == location) %>% 
      ...
      scale_fill_manual(values = c("Seagreen1", "Palegreen3", "Springgreen4" ,"Lawngreen", "Cadetblue3",
                                   "Deepskyblue2", "Aquamarine4", "Yellow1", "Darkslategray1", "Navy", "Snow3"))+
      theme_minimal() +
